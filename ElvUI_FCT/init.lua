...
function FCT:AddOptions(arg1, arg2)
  ...
  if E.Options.args.ElvFCT.args[arg1] and E.Options.args.ElvFCT.args[arg1].args[i] then
    return
  end
  ...
  elseif arg1 == "exclude" then
    ...
    local option = {
      ...
      get = function(info)
        return FCT.db.exclude[arg2][info[#info]]
      end,
      set = function(info, value)
        local which = info[#info]
        if which == "global" then
          if value then
            wipe(FCT.db.exclude[arg2])
          end
          FCT.db.exclude[arg2].global = value or nil
        else
          if value then
            FCT.db.exclude[arg2].global = nil
          end
  ...
  E.Options.args.ElvFCT = {
    ...
    args = {
      ...
      nameplates = {
        ...
        name = L["Nameplates"],
        get = function(info)
          return FCT.db.nameplates[info[#info]]
        end,
        set = function(info, value)
          FCT.db.nameplates[info[#info]] = value
          FCT:UpdateNamePlates()
        end,
      ...
      unitframes = {
        ...
        name = L["UnitFrames"],
        get = function(info)
          return FCT.db.unitframes[info[#info]]
        end,
        set = function(info, value)
          FCT.db.unitframes[info[#info]] = value
          FCT:UpdateUnitFrames()
        end,
        args = {enable = {order = 1, type = "toggle", name = L["Enable"]}}
      },
        ...
        args = {
          remove = {
            ...
            set = function(_, value)
              ...
              if id then
                E.Options.args.ElvFCT.args.exclude.args[id] = nil
              end
            end
          },
          add = {
...
function FCT:BuildProfile()
  local profile = E:CopyTable(ns.defaults)
  copyDefaults(profile.colors, ns.colors)
  for name in pairs(ns.defaults.nameplates.frames) do
    copyDefaults(profile.nameplates.frames[name], ns.frames)
  end
  for name in pairs(ns.defaults.unitframes.frames) do
    copyDefaults(profile.unitframes.frames[name], ns.frames)
  end
  P["elvui_fct"] = profile
end
...
function FCT:SetupProfileHooks()
  E.data.RegisterCallback(self, "OnProfileChanged", "SetupProfile")
  E.data.RegisterCallback(self, "OnProfileCopied", "SetupProfile")
  E.data.RegisterCallback(self, "OnProfileReset", "SetupProfile")
end

function FCT:SetupProfile()
  FCT.db = E.db.elvui_fct
...
function FCT:ToggleFrame(frame)
  ...
  if db then
    FCT:Toggle(frame, info.isNameplate and FCT.db.nameplates or FCT.db.unitframes, db)
  end
end
